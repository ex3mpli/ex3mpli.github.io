...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const uiOverlay = new shaka.ui.Overlay(player, document.getElementById('video-container'), video);
  ...
  async function playChannel(channel) {
    ...
    try {
      if (player.isLive()) await player.unload();
  ...
  function renderChannelList() {
    ...
    document.getElementById('channelCount').textContent = '📺 All Channels: ' + channels.length;
  }
  ...
  window.searchChannels = function () {
    ...
    document.querySelectorAll('.sidebar li').forEach((li) => {
      li.style.display = li.textContent.toLowerCase().includes(searchValue) ? 'list-item' : 'none';
    });
  };
